...
require_once( plugin_dir_path( __FILE__ ) . "include/Custom_Banners_Upgrade_Reminder.class.php" );

/* Blocks */
if ( function_exists('register_block_type') ) {
	require_once( plugin_dir_path( __FILE__ ) . "blocks/banner-list.php" );
	require_once( plugin_dir_path( __FILE__ ) . "blocks/single-banner.php" );
	require_once( plugin_dir_path( __FILE__ ) . "blocks/rotating-banner.php" );
}
...
class CustomBannersPlugin extends CBP_GoldPlugin
{
	...
	function __construct()
	{
		...
		$cur_post_type = isset($_GET['post'])
						 ? sanitize_text_field( get_post_type( intval($_GET['post']) ) )
						 : '';
		...
		if ( !$this->is_pro ) {
			$this->Upgrade_Reminder = new Custom_Banners_Upgrade_Reminder();
		}
	...
	function add_shortcodes()
	{
		...
		if ( function_exists('register_block_type') ) {
			...
			register_block_type( 'custom-banners/banner-list', array(
				'editor_script' => 'banner-list-block-editor',
				'editor_style'  => 'banner-list-block-editor',
				'style'         => 'banner-list-block',
				'render_callback' => array($this, 'banner_shortcode')
			) );

			register_block_type( 'custom-banners/rotating-banner', array(
				'editor_script' => 'rotating-banner-block-editor',
				'editor_style'  => 'rotating-banner-block-editor',
				'style'         => 'rotating-banner-block',
				'render_callback' => array($this, 'banner_shortcode')
			) );

			register_block_type( 'custom-banners/single-banner', array(
				'editor_script' => 'single-banner-block-editor',
				'editor_style'  => 'single-banner-block-editor',
				'style'         => 'single-banner-block',
				'render_callback' => array($this, 'banner_shortcode')
			) );
		}
	}
 ...
 */		
	...
	function provide_config_data_to_admin()
	{
		...
		wp_localize_script( 'single-banner-block-editor', 'custom_banners_admin_single_banner', $translation_array );
		wp_localize_script( 'rotating-banner-block-editor', 'custom_banners_admin_rotating_banner', $translation_array );
		wp_localize_script( 'banner-list-block-editor', 'custom_banners_admin_banner_list', $translation_array );
